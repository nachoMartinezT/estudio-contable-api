...
public class AfipAuthService {
    ...
    public Map<String, String> getAfipToken() throws Exception {
        // 1. Verificar si el token actual sirve (caché)
        if (currentCredentials != null && System.currentTimeMillis() < tokenExpirationTime) {
            return currentCredentials;
        }

        // 2. Si no sirve (o es null), PEDIR UNO NUEVO A AFIP
        System.out.println("🔄 Generando Nuevo Token de AFIP...");
    ...
    private byte[] createLoginTicketRequest() {
        ...
        // CORRECCIÓN: Usar formato ISO estricto para AFIP
    ...
    private String invokeWsaa(byte[] signedCms) throws Exception {
        ...
        if (response.statusCode() != 200) {
            // Dejamos el log para ver errores futuros
            System.out.println("❌ ERROR DE AFIP: " + response.body());
    ...
    private Map<String, String> parseResponse(String xml) {
        // 1. "Des-escapar" el XML. AFIP nos manda &lt; en lugar de <.
        // Reemplazamos los caracteres feos por los bonitos.
        String cleanXml = xml
                .replace("&lt;", "<")
                .replace("&gt;", ">")
                .replace("&quot;", "\"");
        ...
        // 2. Usamos (?s) al principio del Regex para que lea aunque haya saltos de línea (\n)
        ...
        if (mToken.find() && mSign.find()) {
            ...
            // Si falla, mostramos el XML limpio para entender qué pasó
